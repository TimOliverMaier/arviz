(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("2da139cf-99ed-4265-9ece-034db537f460");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '2da139cf-99ed-4265-9ece-034db537f460' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.2.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"d13ed670-7e4f-495e-aaf6-2e80a11b1731":{"defs":[],"roots":{"references":[{"attributes":{"children":[{"id":"17907"},{"id":"17905"}]},"id":"17908","type":"Column"},{"attributes":{},"id":"17891","type":"BasicTickFormatter"},{"attributes":{"data":{},"selected":{"id":"17901"},"selection_policy":{"id":"17900"}},"id":"17881","type":"ColumnDataSource"},{"attributes":{"children":[[{"id":"17832"},0,0]]},"id":"17905","type":"GridBox"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17878","type":"Line"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17857","type":"BoxAnnotation"},{"attributes":{},"id":"17900","type":"UnionRenderers"},{"attributes":{},"id":"17837","type":"LinearScale"},{"attributes":{},"id":"17839","type":"LinearScale"},{"attributes":{},"id":"17842","type":"BasicTicker"},{"attributes":{},"id":"17855","type":"SaveTool"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17877","type":"Line"},{"attributes":{},"id":"17854","type":"UndoTool"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17876","type":"Line"},{"attributes":{"coordinates":null,"formatter":{"id":"17894"},"group":null,"major_label_policy":{"id":"17895"},"ticker":{"id":"17842"}},"id":"17841","type":"LinearAxis"},{"attributes":{"below":[{"id":"17841"}],"center":[{"id":"17844"},{"id":"17848"},{"id":"17874"}],"height":500,"left":[{"id":"17845"}],"output_backend":"webgl","renderers":[{"id":"17872"},{"id":"17879"},{"id":"17885"}],"title":{"id":"17887"},"toolbar":{"id":"17859"},"toolbar_location":null,"width":500,"x_range":{"id":"17833"},"x_scale":{"id":"17837"},"y_range":{"id":"17835"},"y_scale":{"id":"17839"}},"id":"17832","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"17835","type":"DataRange1d"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17899"},"selection_policy":{"id":"17898"}},"id":"17875","type":"ColumnDataSource"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17882","type":"Line"},{"attributes":{},"id":"17895","type":"AllLabels"},{"attributes":{"coordinates":null,"formatter":{"id":"17891"},"group":null,"major_label_policy":{"id":"17892"},"ticker":{"id":"17846"}},"id":"17845","type":"LinearAxis"},{"attributes":{},"id":"17898","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"17858"}},"id":"17853","type":"LassoSelectTool"},{"attributes":{},"id":"17852","type":"WheelZoomTool"},{"attributes":{},"id":"17892","type":"AllLabels"},{"attributes":{},"id":"17833","type":"DataRange1d"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17884","type":"Line"},{"attributes":{},"id":"17850","type":"PanTool"},{"attributes":{"source":{"id":"17868"}},"id":"17873","type":"CDSView"},{"attributes":{"axis":{"id":"17841"},"coordinates":null,"group":null,"ticker":null},"id":"17844","type":"Grid"},{"attributes":{},"id":"17849","type":"ResetTool"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17870","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17883","type":"Line"},{"attributes":{"axis":{"id":"17845"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17848","type":"Grid"},{"attributes":{"overlay":{"id":"17857"}},"id":"17851","type":"BoxZoomTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17871","type":"Line"},{"attributes":{"tools":[{"id":"17849"},{"id":"17850"},{"id":"17851"},{"id":"17852"},{"id":"17853"},{"id":"17854"},{"id":"17855"},{"id":"17856"}]},"id":"17859","type":"Toolbar"},{"attributes":{"toolbar":{"id":"17906"},"toolbar_location":"above"},"id":"17907","type":"ToolbarBox"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17869","type":"Line"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17858","type":"PolyAnnotation"},{"attributes":{},"id":"17901","type":"Selection"},{"attributes":{"callback":null},"id":"17856","type":"HoverTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17881"},"glyph":{"id":"17882"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17884"},"nonselection_glyph":{"id":"17883"},"view":{"id":"17886"}},"id":"17885","type":"GlyphRenderer"},{"attributes":{},"id":"17896","type":"UnionRenderers"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17874","type":"BoxAnnotation"},{"attributes":{"coordinates":null,"data_source":{"id":"17868"},"glyph":{"id":"17869"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17871"},"nonselection_glyph":{"id":"17870"},"view":{"id":"17873"}},"id":"17872","type":"GlyphRenderer"},{"attributes":{},"id":"17894","type":"BasicTickFormatter"},{"attributes":{},"id":"17899","type":"Selection"},{"attributes":{"coordinates":null,"data_source":{"id":"17875"},"glyph":{"id":"17876"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17878"},"nonselection_glyph":{"id":"17877"},"view":{"id":"17880"}},"id":"17879","type":"GlyphRenderer"},{"attributes":{"toolbars":[{"id":"17859"}],"tools":[{"id":"17849"},{"id":"17850"},{"id":"17851"},{"id":"17852"},{"id":"17853"},{"id":"17854"},{"id":"17855"},{"id":"17856"}]},"id":"17906","type":"ProxyToolbar"},{"attributes":{},"id":"17846","type":"BasicTicker"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17897"},"selection_policy":{"id":"17896"}},"id":"17868","type":"ColumnDataSource"},{"attributes":{},"id":"17897","type":"Selection"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17887","type":"Title"},{"attributes":{"source":{"id":"17875"}},"id":"17880","type":"CDSView"},{"attributes":{"source":{"id":"17881"}},"id":"17886","type":"CDSView"}],"root_ids":["17908"]},"title":"Bokeh Application","version":"2.4.2"}}';
                  const render_items = [{"docid":"d13ed670-7e4f-495e-aaf6-2e80a11b1731","root_ids":["17908"],"roots":{"17908":"2da139cf-99ed-4265-9ece-034db537f460"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();